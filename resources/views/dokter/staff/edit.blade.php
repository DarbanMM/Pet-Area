...
@section('title', 'Edit Admin - Dokter')
@section('header_title', 'Edit Admin')

@section('content')
    <h2 class="text-3xl font-semibold text-gray-800 mb-6">Edit Data Admin: {{ $staff->name }}</h2>
    ...
    <form action="{{ route('dokter.staff.update', $staff->id) }}" method="POST" class="bg-white rounded-lg shadow-md p-6 max-w-xl mx-auto">
        @csrf
        @method('PUT')
        <div class="mb-4">
