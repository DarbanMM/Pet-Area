{{-- Memilih layout master berdasarkan peran pengguna --}}
    ...
    <div class="container mx-auto mt-8 p-6 bg-white rounded-lg shadow-md max-w-3xl">
        ...
        <div class="flex justify-between mt-8">
            ...
            @else {{-- Dokter --}}
                ...
                {{-- Dokter juga punya tombol edit/delete di sini, tampilkan hanya jika user adalah dokter --}}
                @can('manage-medical-records') {{-- Gate ini akan memastikan hanya dokter yang bisa edit/hapus --}}
                    <div class="space-x-2">
                        ...
                        <form action="{{-- route('dokter.medical_records.destroy', $medicalRecord->id) --}}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus rekam medis ini?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-md transition duration-150">Hapus</button>
                        </form>
                    </div>
                @endcan
            @endif
